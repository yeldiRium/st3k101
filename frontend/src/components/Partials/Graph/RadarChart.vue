...
export default {
  ...
  computed: {
    ...
    minValue() {
      ...
      return Math.min(
        ...map(prop("questionRangeStart"), this.questionStatistics)
      );
    },
    maxValue() {
      if (this.questionStatistics.length < 1) {
      ...
      return Math.max(
        ...map(prop("questionRangeEnd"), this.questionStatistics)
      );
    }
  },
  created() {
  ...
  mounted() {
    this.chart = new Chart(document.getElementById(this.id), this.graphConfig);
  }
};
</script>
