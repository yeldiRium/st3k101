...
const store = {
  ...
  getters: {
    ...
    myQuestionnaires(state, getters, rootState, rootGetters) {
      ...
      if (!isNil(dataClient)) {
        const questionnaires = pipe(
          filter(questionnaire => questionnaire.isOwnedBy(dataClient)),
          map(clone)
    ...
    questionnaireById(state, getters, rootState, rootGetters) {
      return function(id) {
        const questionnaire = clone(
          find(questionnaire => questionnaire.id === id, state.questionnaires)
        );
  ...
  actions: {
    ...
    removeQuestionnaireFromStore({ commit, dispatch }, { questionnaire }) {
      return Future((reject, resolve) => {
        ...
        return Future.parallel(Infinity, removeDimensionFutures).map(
          () => true
        );
      });
    },
    ...
    createConcreteQuestionnaire(
      ...
      { language, name, description, isPublic, allowEmbedded, xapiTarget }
    ) {
      return createConcreteQuestionnaire(
        ...
        dispatch("patchQuestionnaireInStore", { questionnaire })
      );
    },
    /**
    ...
    createShadowQuestionnaire(
      ...
      return createShadowQuestionnaire(
        ...
        dispatch("patchQuestionnaireInStore", {
    ...
    fetchQuestionnaire(
      ...
      return future.chain(questionnaire =>
        dispatch("patchQuestionnaireInStore", { questionnaire })
      );
    },
    /**
      ...
      return fetchQuestionnaireTemplates(
        ...
        for (const template of templates) {
          patchTemplateFutures.push(
            dispatch("patchQuestionnaireInStore", { questionnaire: template })
          );
        }
    ...
    updateQuestionnaire(
      ...
      return apiUpdateQuestionnaire(
        ...
        dispatch("patchQuestionnaireInStore", { questionnaire: result })
      );
    },
    /**
    ...
    deleteQuestionnaire({ dispatch, rootGetters }, { questionnaire }) {
      return (
        deleteQuestionnaire(rootGetters["session/sessionToken"], questionnaire)
          .chain(() =>
            dispatch("removeQuestionnaireFromStore", { questionnaire })
          )
          // TODO: remove this
          // Remove the questionnaire even if the API throws an error.
          // This is used for testing while the API is not ready yet.
          .chainRej(error => {
            return dispatch("removeQuestionnaireFromStore", {
              questionnaire
    ...
    addShadowDimension(
      { commit, dispatch, rootGetters },
      { questionnaire, concreteDimension }
    ) {
      if (questionnaire.isShadow) {
        return Future.reject(
          new ValidationError(
      ...
      return addShadowDimension(
        ...
        return (
          dispatch(
            ...
            // reference count
            .chain(() =>
              dispatch("dimensions/fetchDimension", {
    ...
    removeDimension(
      ...
      return removeDimension(
        ...
        commit("removeDimensionFromQuestionnaire", {
          questionnaire,
          dimension
        });
        return dispatch(
  ...
  mutations: {
    ...
    addDimensionToQuestionnaire(state, { questionnaire, dimension }) {
      ...
      if (!isNil(questionnaireInStore)) {
        ...
        questionnaireInStore.dimensions = uniq(questionnaireInStore.dimensions);
      }
    },
    /**
     * Removes a Dimension from a ConcreteQuestionnaire.
