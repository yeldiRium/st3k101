...
class TrackerEntry {
   ...
   */
  constructor(dataclientEmail, timestamp) {
 ...
 * was changed.
 */
class TranslatedPropertyUpdatedTrackerEntry extends PropertyUpdatedTrackerEntry {
  /**
   *
   * @param dataclientEmail {String}
   * @param timestamp {Date}
  ...
  toString() {
    return `[${this.type} ${this.timestamp.getTime().toString()} ${
      this.itemHref
    } ${this.propertyName} ${this.language}]`;
  }
}
...
class QuestionnaireRemovedTrackerEntry extends TrackerEntry {
  constructor(dataclientEmail, timestamp, questionnaireName) {
    super(dataclientEmail, timestamp);
